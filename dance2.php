...
<body style="background-color: #C3C3E5; color: #443266">
	<header>
		<nav class="navbar navbar-expand-md navbar-dark fixed-top bg-dark">
			<a href="luna.html" class="navbar-brand">Krieva</a>
		</nav>	
	</header>
				...
				<main>
                    ...
                    if($response) {
                        while ($row = mysqli_fetch_array($response)) {
                            if($row['event_name']=='Dance')
                                echo '<section><h2 id = "Starter">'.$row['details'].'</h2></section>';
                        }
                    }
                    require_once ("temp.php");
                    $query = 'SELECT list, description FROM dance';
                    $response = @mysqli_query($dbc, $query);
                    if($response) {
                        while ($row = mysqli_fetch_array($response)) {
